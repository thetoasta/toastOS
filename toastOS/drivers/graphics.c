 ...
 *
 * Implements double-buffered rendering with an 8x16 bitmap font,
 * pixel primitives, and alpha blending for the toastGUI compositor.
 */
...
/*  State                                                              */
/* ------------------------------------------------------------------ */
static uint32_t *lfb       = 0;    /* linear framebuffer (VRAM)       */
static uint32_t *backbuf   = 0;    /* system-RAM backbuffer           */
static uint32_t  scr_w     = 0;
static uint32_t  scr_h     = 0;
static uint32_t  scr_pitch = 0;    /* bytes per scanline              */
static int       ready     = 0;
...
/* ------------------------------------------------------------------ */
static void fast_copy32(uint32_t *dst, const uint32_t *src, uint32_t count) {
    while (count >= 8) {
...
static void fast_set32(uint32_t *dst, uint32_t val, uint32_t count) {
    while (count >= 8) {
...
void gfx_init(uint32_t *framebuffer, uint32_t width, uint32_t height, uint32_t pitch) {
    ...
    backbuf = (uint32_t *)kmalloc(width * height * 4);
    if (!backbuf) return;
...
int gfx_is_ready(void) { return ready; }

uint32_t *gfx_get_backbuffer(void) { return backbuf; }
...
void gfx_flush(void) {
    ...
    for (uint32_t y = 0; y < scr_h; y++) {
        uint32_t *dst = (uint32_t *)((uint8_t *)lfb + y * scr_pitch);
...
void gfx_draw_rect_alpha(int x, int y, int w, int h, uint32_t color) {
    ...
    for (int row = y; row < y + h; row++) {
        ...
        int x1 = (x + w) > (int)scr_w ? (int)scr_w : (x + w);
        for (int col = x0; col < x1; col++)
            line[col] = gfx_blend(color, line[col]);
    }
}
...
void gfx_draw_outline(int x, int y, int w, int h, uint32_t color, int thickness) {
    if (!ready) return;
    /* Top */
    gfx_draw_rect(x, y, w, thickness, color);
    /* Bottom */
    gfx_draw_rect(x, y + h - thickness, w, thickness, color);
    /* Left */
    gfx_draw_rect(x, y + thickness, thickness, h - 2 * thickness, color);
    /* Right */
    gfx_draw_rect(x + w - thickness, y + thickness, thickness, h - 2 * thickness, color);
}
...
void gfx_draw_char(int x, int y, char c, uint32_t fg, uint32_t bg) {
    ...
    for (int row = 0; row < FONT_H; row++) {
        int sy = y + row;
        if ((unsigned)sy >= scr_h) break;
        if (sy < 0) continue;
        uint8_t bits = glyph[row];
        uint32_t *line = backbuf + sy * scr_w;
        for (int col = 0; col < FONT_W; col++) {
            int sx = x + col;
            if ((unsigned)sx >= scr_w) break;
            if (sx < 0) continue;
...
void gfx_draw_string(int x, int y, const char *str, uint32_t fg, uint32_t bg) {
    while (*str) {
        if (*str == '\n') {
            y += FONT_H;
            x = 0; /* caller should manage x if needed */
        } else {
...
void gfx_blit(int dx, int dy, int w, int h, const uint32_t *src) {
    ...
    for (int row = 0; row < h; row++) {
        ...
        int x0 = dx < 0 ? -dx : 0;
        int x1 = (dx + w) > (int)scr_w ? (int)scr_w - dx : w;
        for (int col = x0; col < x1; col++)
            dst[dx + col] = src[col];
        src += w;
    }
