...
/* ========== toast::net namespace implementations ========== */
namespace toast {
namespace net {

int init() { return net_init(); }
int ping(const char* host) { return net_ping(host); }
int http_get(const char* host, const char* path) { return net_http_get(host, path); }
int browse(const char* url) { return net_browse(url); }
void print_local_ip() { net_print_local_ip(); }
void get_info(Info* info) { net_get_info(info); }
