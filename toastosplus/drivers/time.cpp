...
/* ========== toast::time namespace implementations ========== */
namespace toast {
namespace time {

void init() { init_timer(); }
void tick() { timer_handler(); }
time_t now() { return get_time(); }
void update_bar() { update_top_bar(); }
void set_timezone(int offset_hours) { ::set_timezone(offset_hours); }
int get_timezone() { return ::get_timezone(); }
void set_24hr(int is_24hr) { set_time_format(is_24hr); }
int is_24hr() { return get_time_format(); }
uint32_t uptime() { return get_uptime_seconds(); }

namespace alarm {
    int set(uint8_t hour, uint8_t minute, const char* note) { return alarm_set(hour, minute, note); }
    void clear(int index) { alarm_clear(index); }
    void clear_all() { alarm_clear_all(); }
    int count() { return alarm_count(); }
    const Alarm* get(int index) { return alarm_get(index); }
    void check() { alarm_check(); }
}
